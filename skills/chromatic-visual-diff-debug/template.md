# Visual Diff Debug Intake

## Minimal customer request

Please share the Chromatic build URL or build number, the affected story name or story ID, and one screenshot or diff link that shows the unexpected change.

## Local reproduction check

Please confirm whether the same issue appears in local Storybook. If it does, say that explicitly.

## Minimal config snippet request

If you already suspect globals, assets, or timing, please share the relevant story snippet plus any `.storybook/preview.*` parameters that affect rendering.
